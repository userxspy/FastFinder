...
# 🚀 FIX: Updated authentication to check BOTH Admin and Public Users
def is_auth(req): 
    # Check Admin
    s = req.cookies.get('admin_session')
    if s and hasattr(temp, 'ADMIN_SESSIONS') and s in temp.ADMIN_SESSIONS and time.time() < temp.ADMIN_SESSIONS[s]:
        return True
    
    # Check Public User
    u = req.cookies.get('user_session')
    if u and hasattr(temp, 'PUBLIC_SESSIONS') and temp.PUBLIC_SESSIONS.get(u, {}).get("expire", 0) > time.time():
        return True
        
    return False

# 🚀 FIX: Updated dynamic logout link based on user type
def build_page(title, body, cls="", ad="", as_="", is_public_user=False):
    logout_url = "/user_logout" if is_public_user else "/logout"
    
    nav = f'<div class="sidebar-overlay" id="sbOverlay" onclick="closeSidebar()"></div><div class="sidebar" id="sidebar"><div class="sb-header"><div class="sb-logo"><span class="nf-icon">F</span> FAST FINDER</div><button class="sb-close" onclick="closeSidebar()">&#10005;</button></div><nav class="sb-nav"><div class="sb-section">Menu</div><a href="/dashboard" class="sb-link {ad}">Home</a><a href="/stats" class="sb-link {as_}">Database Stats</a></nav><div class="sb-footer"><a href="{logout_url}" class="sb-logout">Sign Out</a></div></div><div class="topbar"><button class="ham-btn" id="hamBtn" onclick="openSidebar()"><span class="ham-line"></span><span class="ham-line"></span><span class="ham-line"></span></button><a class="logo" href="/dashboard"><span class="nf-icon">F</span> FAST FINDER</a><div class="topbar-right"><button class="theme-btn" onclick="toggleTheme()">Theme</button></div></div>' if ad or as_ else ""
...
async def dash(req):
    if not is_auth(req): return web.HTTPFound('/login') # Redirects public users to /login instead of /admin
    
    # Check if the current user is a public user (to set correct logout link)
    is_public = bool(req.cookies.get('user_session'))
    ...
    return build_page("Home - Fast Finder", b, "", "active", "", is_public)

@admin_routes.get('/stats')
async def stats(req):
    if not is_auth(req): return web.HTTPFound('/login')
    
    is_public = bool(req.cookies.get('user_session'))
    
    try: 
        s = await db_count_documents()
        s_dict = {'total':s,'primary':s,'cloud':0,'archive':0} 
    except: 
        s_dict = {'total':0,'primary':0,'cloud':0,'archive':0}
        
    try: u = await user_db.total_users_count()
    except: u = 0
    
    b = f'<div class="main" style="padding-top:40px;"><div class="big-stat"><div class="big-stat-val">{s_dict.get("total",0):,}</div><div class="big-stat-label">Total Titles Available</div></div><div class="stats-row"><div class="scard red"><div class="scard-label">Movies</div><div class="scard-val">{s_dict.get("primary",0):,}</div><div class="scard-sub">Primary source</div></div><div class="scard white"><div class="scard-label">Series</div><div class="scard-val">{s_dict.get("cloud",0):,}</div><div class="scard-sub">Cloud storage</div></div><div class="scard grey"><div class="scard-label">Archive</div><div class="scard-val">{s_dict.get("archive",0):,}</div><div class="scard-sub">Backup library</div></div><div class="scard red"><div class="scard-label">Profiles</div><div class="scard-val">{u:,}</div><div class="scard-sub">Active watchers</div></div></div></div>'
    return build_page("Stats - Fast Finder", b, "", "", "active", is_public)
