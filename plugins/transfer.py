        ...
        for chunk_start in range(last_id + 1, max_id + 1, chunk_size):
            ...
            for msg in messages:
                ...
                if not msg or msg.empty:
                    ...
                    # ✅ सिर्फ लिंक (URLs) निकालने का लॉजिक
                    extracted_links = []
                    ...
                    for ent in entities:
                        if ent.type == enums.MessageEntityType.URL:
                            extracted_links.append(text[ent.offset : ent.offset + ent.length])
                        elif ent.type == enums.MessageEntityType.TEXT_LINK:
                            extracted_links.append(ent.url)

                    # अगर मैसेज में लिंक मिले हैं, तो ही आगे बढ़ें
                    if extracted_links:
                        # अगर एक मैसेज में 2-3 लिंक हैं, तो उन्हें एक साथ जोड़कर भेजेंगे
                        links_to_send = "\n".join(extracted_links)
                        
                        try:
                            await user_app.send_message(tgt_chat, text=links_to_send, disable_web_page_preview=True)
                            state["copied"] += 1
                            await asyncio.sleep(30) # ✅ 30 सेकंड का फिक्स अंतराल
                            ...
                            logger.error(f"Error sending link for msg {msg_id}: {e}")
                            state["skipped"] += 1
                    else:
                        state["skipped"] += 1
            ...
            if (chunk_start // chunk_size) % 5 == 0:
                try: await status_msg.edit(f"⏳ **Transferring Links...**\n\n📊 Scanned: {state['scanned']}\n✅ Links Sent: {state['copied']}\n⏭ Skipped (No Links): {state['skipped']}\n\n📍 Last Processed ID: {state['last_id']}")
                except: pass

        await user_app.stop()
        if is_transferring:
            await message.reply(f"🎉 **Link Transfer Complete!**\n\n📊 Final Stats:\nScanned Messages: {state['scanned']}\nLinks Sent: {state['copied']}\nSkipped: {state['skipped']}")
